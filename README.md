...
脚本会识别架构、校验 GitHub Release、安装二进制和 `mosdns.service`，然后启动服务；中国大陆出口默认通过 `gh-proxy.com` 下载 Release，但 `SHA256SUMS` 始终直连 GitHub 获取，不经过代理。它不会安装 Caddy、创建服务账户或覆盖已有配置。详细说明见[一键安装](docs/installation.md)。
