...
func TestMySQLDNSPolicySettingsReadAndUpdate(t *testing.T) {
	...
	columns := []string{"user_id", "strip_ecs", "block_private_answers", "blocked_qtypes_json", "custom_block_enabled", "custom_allow_enabled", "custom_rewrite_enabled", "policy_paused_until_ns", "answer_family", "ttl_min", "ttl_max", "flatten_cname", "shuffle_answers", "updated_at_ns"}
	storedPause := now.Add(30 * time.Minute)
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT ` + mysqlDNSPolicySettingsColumns + ` FROM mosdns_dns_policy_settings WHERE user_id=?`)).WithArgs("user-1").WillReturnRows(sqlmock.NewRows(columns).AddRow("user-1", false, false, []byte(`[]`), true, true, true, storedPause.UnixNano(), "ipv4", uint32(30), uint32(0), true, false, now.Add(-time.Hour).UnixNano()))
	...
	if settings.AnswerFamily != AnswerFamilyIPv4 || settings.TTLMin != 30 || !settings.FlattenCNAME || settings.ShuffleAnswers {
		t.Fatalf("additive columns read back wrong: %+v", settings)
	}

	mock.ExpectBegin()
	expectMySQLPolicyOwner(mock, now)
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT ` + mysqlDNSPolicySettingsColumns + ` FROM mosdns_dns_policy_settings WHERE user_id=? FOR UPDATE`)).WithArgs("user-1").WillReturnRows(sqlmock.NewRows(columns).AddRow("user-1", false, false, []byte(`[]`), true, true, true, storedPause.UnixNano(), "ipv4", uint32(30), uint32(0), true, false, now.Add(-time.Hour).UnixNano()))
	pausedUntil := now.Add(time.Hour)
	mock.ExpectExec(regexp.QuoteMeta(`UPDATE mosdns_dns_policy_settings SET strip_ecs=?, block_private_answers=?, blocked_qtypes_json=?, custom_block_enabled=?, custom_allow_enabled=?, custom_rewrite_enabled=?, policy_paused_until_ns=?, answer_family=?, ttl_min=?, ttl_max=?, flatten_cname=?, shuffle_answers=?, updated_at_ns=? WHERE user_id=?`)).WithArgs(true, false, []byte(`["AAAA","A"]`), false, true, true, pausedUntil.UnixNano(), "ipv4", uint32(30), uint32(0), true, false, now.UnixNano(), "user-1").WillReturnResult(sqlmock.NewResult(0, 1))
	...
	// A patch that does not name answer_family must keep the stored value.
	if settings.AnswerFamily != AnswerFamilyIPv4 {
		t.Fatalf("answer_family = %q after an unrelated update, want ipv4 preserved", settings.AnswerFamily)
	}
...
func TestInsertMySQLDNSPolicySettingsPersistsAllFields(t *testing.T) {
	...
	mock.ExpectExec(regexp.QuoteMeta(`INSERT INTO mosdns_dns_policy_settings`)).WithArgs("user-1", true, true, []byte(`["AAAA"]`), false, true, false, pausedUntil.UnixNano(), "ipv6", uint32(0), uint32(600), false, true, now.UnixNano()).WillReturnResult(sqlmock.NewResult(1, 1))
	err = insertMySQLDNSPolicySettings(context.Background(), tx, DNSPolicySettings{
		...
		TTLMax: 600, ShuffleAnswers: true, UpdatedAt: now,
	})
	if err != nil {
		t.Fatal(err)
	}
